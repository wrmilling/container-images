...
# if ENV TZ is set, change the value in appdaemon.yaml
if [ -n "$TZ" ]; then
  sed -i "s/^  time_zone:.*/  time_zone: $(echo $TZ | sed -e 's/\\/\\\\/g; s/\//\\\//g; s/&/\\\&/g')/" $CONF/appdaemon.yaml
fi
...
# Lets run it!
exec appdaemon -c $CONF "$@" ${EXTRA_ARGS}
